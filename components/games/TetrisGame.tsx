...
import { TetrisEngine, H, W } from "@/lib/games/tetris/engine";
import type { GameComponentProps, GameEngineHandle } from "@/lib/games/registry";

export function TetrisGame({ onSnapshot, onReady }: GameComponentProps) {
  ...
  useEffect(() => {
    ...
    const engine = new TetrisEngine(ctx);
    engine.init();

    let rafId = 0;
    let lastTime: number | null = null;

    const GAME_KEYS = new Set(["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "Space", "KeyP"]);
